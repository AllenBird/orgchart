 ...
 * Author: Mark Lee
 * User: Allen DP
 * Copyright (C)2015-2015 DT Dream
...
(function($) {
  ...
  $.fn.orgChart = function(options) {
    ...
    /* 如果没有定义container，则覆盖ul */
    if (undefined == opts.container){
      opts.container = $(this);
    }
    /* 建立并插入有序列表 */
    var data = [];
  ...
  $.fn.orgChart.defaults = {
    ...
    showLevels : -1,
    stack      : false,
    chartClass : "orgChart panel panel-default",
    hoverClass : "hover",
  ...
  function buildNode($node, $appendTo, level, index, opts) {
    ...
    if (level >= 3) {
      var $heading = $("<h3>").html(opts.nodeText($node));
    }
    else{
